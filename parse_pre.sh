...
JAVA_FILES_DIR="src/org/mtransit/parser/ca_york_region_yrt_viva_bus";
JAVA_STOPS_FILE="$JAVA_FILES_DIR/Stops.java";
echo ">> Pre Parsing > Set Java stops file...";
> $JAVA_STOPS_FILE; # empty file
echo "package org.mtransit.parser.ca_york_region_yrt_viva_bus;" >> $JAVA_STOPS_FILE;
...
awk -F "\"*,\"*" '{print "		allStops.put(\"" $3 "\", \"" $11"\"); // " $9}' input/gtfs/stops.txt >> $JAVA_STOPS_FILE;
checkResult $? false;
